/**
 * World Buffs Google Apps Script
 * 
 * This script populates world buffs data in the "world buffs" tab
...
// Configuration
const SPREADSHEET_ID = '1kjhKMlJrUJu_1kyeWoNV4TmKQs5rIZLY0p_pQyjiXFM';
...
/**
 * Main doPost function - handles incoming requests for both World Buffs and Backup actions
 */
function doPost(e) {
  try {
 ...
 * This function is not called remotely but can be used for testing
 */
function testPopulateWorldBuffs() {
  try {
    const spreadsheet = SpreadsheetApp.openById(SPREADSHEET_ID);
    const worldBuffsSheet = spreadsheet.getSheetByName(WORLD_BUFFS_TAB_NAME);
    ...
    populateWorldBuffs(worldBuffsSheet, testLogUrl);
